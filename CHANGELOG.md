...
Manuscript-to-repository alignment (branch
`paper4-manuscript-repo-alignment`). Under the methodological rule
**repository = source of technical truth, paper adapts to the
reproducible repository**, the LaTeX manuscript `paper/main_v4.tex`
is updated in place so every path, table, figure, script name and
quantitative claim is coherent with the pinned state of the
`DAV-IDE/paper4-esorics` repository at v4.1.1.

No repository scripts, results JSONs or generated figures are
modified in a way that changes any measured number; only the paper
text, the LaTeX comments, the header of `tables/real_data.tex`
(inserted by `scripts/build_table_and_pareto.py`), and the header of
`scripts/build_table_and_pareto.py` itself are touched.

### Empirical findings reported (not silently edited)

- **Table 7 `tab:headline` (§5.1) — FP counts reconciled with
  `results/synthetic/results_ready_summary.csv`.** The four rows of the
  headline table previously reported FP\_A / FP\_pre values that do
  not match the pinned 45-run factorial CSV. They are updated as
  follows (delay $d$ and best-$(E,C)$ pairs unchanged):
  - Drift(B) $(\Hsh, C_1)$: FP\_A / FP\_pre  **82 / 93 → 65 / 31**
  - Drift(B) $(\Hsh, C_3)$: FP\_A / FP\_pre  **27 / 60 → 69 / 27**
  - Shock(C) $(\Hpe, C_1)$: FP\_A / FP\_pre  **26 / 140 → 38 / 40**
  - Shock(C) $(\Hsh, C_2)$: FP\_A / FP\_pre  **82 / 200 → 82 / 52**

  The FP\_A column now equals the pre-event false-positive count of
  the corresponding stable-regime row (Scenario A, matching $C$ and
  $E$) in the CSV, and FP\_pre equals the pre-event count of the
  attack-regime row (Scenario B or C, matching $C$ and $E$). The
  ranking, the interpretation paragraph, and the design-space thesis
  are unaffected.
- **§5.4 SPARQL scaling narrative reconciled with
  `tables/sparql_scaling.tex`.** The interpretation paragraph
  previously cited Q1 11.6→345 ms, Q2 11.9→466 ms, Q3 4.6→66.7 ms
  from an earlier draft rdflib configuration. The paragraph is
  rewritten with the current repository values Q1 4.17→109.90 ms,
  Q2 4.99→189.01 ms, Q3 1.77→23.70 ms (growth factors ~26× / ~38× /
  ~13× over the 50× range 10^3–5·10^4 triples). The relative ordering
  Q3 < Q1 < Q2 and the sub-quadratic growth regime are unchanged. The
  under-100-ms operational statement is re-scoped to the 10^4-triple
  range only; at 5·10^4 triples Q1 crosses 100 ms (~110 ms) and Q2
  approaches 190 ms, which the paragraph now states explicitly.

### Changed

- `paper/main_v4.tex` — obsolete path prefix `experiments/day1/` fully
  removed from the manuscript (comment header, §5.2, and §5.4 Table
  caption). The reproducibility instructions in the header block now
  reference the flat top-level `scripts/` directory that is the actual
  repository layout.
- `paper/main_v4.tex` §5.2 — line count for the transparent statistical
  baseline updated from **352 → 401 lines** to match
  `scripts/baseline_zscore.py` after the sklearn AUC support additions
  of v4.1.1; the reference to `scikit-learn` is made explicit in text.
- `paper/main_v4.tex` §5.2 — a new caveat paragraph inserted between
  Table 8 (`tab:real-data`) and the "Reading the table" paragraph.
  The caveat states that the AUC\_ROC / AUC\_PR values of Table 8
  (roughly 0.40–0.55 for AUC-ROC, 0.24–0.31 for AUC-PR) are diagnostic
  of the transparent statistical baseline and the proxy EAL pipeline
  on a single targeted CIC-IDS2017-aligned subset, and are
  **not** a detection-performance claim of Paper 4. It reasserts
  Boundary B1 of §5.6: the inherited AUC≈0.993 / FPR≈0.021 of
  `\cite{paper3}` is not superseded by the numbers of Table 8.
- `paper/main_v4.tex` Reproducibility Statement — explicit designation
  of `DAV-IDE/paper4-esorics` as the primary reproducibility artefact
  for the present paper (with reference to `REPRODUCIBILITY.md` and
  `CHANGELOG.md`), and honest note that each upstream companion
  repository carries its own licence and that licence coverage is
  tracked in the Paper 4 repository.
- `scripts/build_table_and_pareto.py` — auto-generated header of
  `tables/real_data.tex` corrected: obsolete `experiments/day1/`
  source path removed, and the AUC caveat ("diagnostic, not a
  detection-performance claim of Paper 4, does not supersede Paper 3
  inherited AUC/FPR") added as a comment block above the tabular so
  that the caveat travels with the file even outside the LaTeX build.
- `tables/real_data.tex` — regenerated by `make table` after the
  script edit; numeric values (AUC\_ROC, AUC\_PR, FPR\_A, Delay,
  Fires, Detected) are byte-identical to v4.1.1; only the header
  comment lines change.

### Preserved

- No new detection-performance claim.
- No forbidden E.3 wording introduced (checked against `docs/wording_audit.md`).
- All Boundary B1/B2/B3 statements, threats to validity, and scope
  choices are kept verbatim.
- Inherited Paper 3 values (AUC≈0.993, FPR≈0.021, 88 % MSU reduction,
  latency 25.8 ms) are cited only as inherited, never as new results.
- The Pareto-front figure and its interpretation are unchanged.
