...
def aggregate_per_minute(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = []
    for m, g in grouped:
        pkt = float(g["Flow Packets/s"].replace([np.inf, -np.inf], np.nan).fillna(0).sum())
        byte = float(g["Flow Bytes/s"].replace([np.inf, -np.inf], np.nan).fillna(0).sum())
    ...
    out = pd.DataFrame(rows, columns=["__min", "pkt_rate", "byte_rate", "dport_entropy", "label"])
...
def run_zscore(per_time: pd.DataFrame, feature_col: str, cfg: Config) -> pd.DataFrame:
    ...
    for _, row in per_time.iterrows():
        ...
        if t < cfg.warmup_seconds or n < 2 or sigma < eps or math.isnan(sigma):
            mus.append(mu); sigmas.append(sigma); zs.append(float("nan")); fires.append(0)
        else:
            z = (x - mu) / sigma
            mus.append(mu); sigmas.append(sigma); zs.append(z); fires.append(int(z > cfg.k))
...
def compute_metrics(df: pd.DataFrame, feature_col: str, cfg: Config) -> dict:
    ...
    try:
        ...
        auc_pr  = float(average_precision_score(y_m, z_m)) if len(set(y_m)) > 1 else float("nan")
    except Exception:
    ...
    return {
        ...
        "FPR_at_k3": fpr,
        "detected": detected, "detection_delay_s": delay,
        ...
        "k": cfg.k, "window_seconds": cfg.window_seconds,
...
def main(cfg: Config) -> None:
    ...
    LOG.info("Per-minute rows: %d, attack-minutes: %d", len(per_time), int(per_time["label"].sum()))

    features = [("pktrate", "pkt_rate"),
                ("byterate", "byte_rate"),
                ("dportentropy", "dport_entropy")]
    all_metrics = {}
    for short, col in features:
        enriched = run_zscore(per_time, col, cfg)
        (cfg.output_dir / f"baseline_zscore_{short}.csv").write_text(enriched.to_csv(index=False))
        m = compute_metrics(enriched, col, cfg)
        (cfg.output_dir / f"baseline_zscore_{short}_metrics.json").write_text(json.dumps(m, indent=2))
        all_metrics[short] = m
        LOG.info("  %s: AUC_ROC=%.4f FPR=%.4f delay=%s fires=%d",
                 col, m["AUC_ROC"], m["FPR_at_k3"], m["detection_delay_s"], m["n_fires_post_warmup"])

    (cfg.output_dir / "baseline_zscore_summary.json").write_text(json.dumps(all_metrics, indent=2))
    LOG.info("Summary written")


def parse_args() -> Config:
    p = argparse.ArgumentParser(description="Causal rolling z-score baseline on real CIC-IDS2017 (native minute)")
    ...
    a = p.parse_args()
    return Config(input_path=a.input, output_dir=a.output,
                  warmup_seconds=a.warmup, window_seconds=a.window, k=a.k)
