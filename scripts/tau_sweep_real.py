...
def main() -> None:
    ...
    print(
        f"[tau-sweep] per-minute rows={len(per_time)}, "
    ...
    for warmup in WARMUP_VALUES:
        ...
        for k in K_VALUES:
            ...
            for est, W, s in NATIVE_MIN_GRID:
                ...
                tau_str = "inf" if math.isinf(m["tau"]) else f"{m['tau']:.3f}"
                print(
                    f"  {est:11s} W={W:2d} s={s} | "
                    f"tau={tau_str:>7s}  "
                    f"AUC={m['AUC_ROC']:.3f}  "
            ...
            (by_k_dir / f"warmup{warmup}_k{k}.json").write_text(
                json.dumps(rows_k, indent=2)
            )
