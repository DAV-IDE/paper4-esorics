...
def run_eal(per_time: pd.DataFrame, cfg: EALConfig) -> tuple[pd.DataFrame, dict]:
    ...
    stable_mask = (per_time["t"].to_numpy() < cfg.warmup_s) & (per_time["label"].to_numpy() == 0)
    ...
    fires = (~np.isnan(eps_series)) & (eps_series > tau) & (per_time["t"].to_numpy() >= cfg.warmup_s)
    ...
    try:
        ...
        auc_pr  = float(average_precision_score(y_m, h_m)) if len(set(y_m)) > 1 else float("nan")
    except Exception:
    ...
    return out, {
        ...
        "W": cfg.W, "s": cfg.s, "k": cfg.k,
        "mu_stable": mu, "sigma_stable": sigma, "tau": tau,
...
def main():
    ...
    p.add_argument("--native-min", action="store_true",
    ...
    print(f"[eal-real] per-minute rows={len(per_time)}, attack-minutes={int(per_time['label'].sum())}")
    ...
    for est, W, s in GRID:
        ...
        print(f"      AUC_ROC={m['AUC_ROC']:.4f}  FPR={m['FPR_post_warmup']:.4f}  "
