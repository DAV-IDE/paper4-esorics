...
def find(warmup: int, k: float, est: str, W: int, s: int) -> dict:
    for r in rows:
        if (r["warmup_s"] == warmup and r["k"] == k
                and r["estimator"] == est and r["W"] == W and r["s"] == s):
            return r
...
lines.append(r"\caption{Threshold recalibration sweep on real CIC-IDS2017 "
...
header1 = [r"", r"", r"", r"AUC"]
for w in WARMUPS:
    header1.append(r"\multicolumn{4}{c|}{warmup=" + str(w) + r"s (" + f"{w//60}" + r" min pre-atk)}")
...
lines.append(r"\emph{Reading key.} A cell shows how many post-warmup minutes "
...
# ---------------------------------------------------------------------------
fig, axes = plt.subplots(1, 3, figsize=(14.0, 5.6), dpi=150,
...
for ax, warmup in zip(axes, WARMUPS):
    ...
    for i, (est, W, s) in enumerate(CONFIGS):
        for j, k in enumerate(KS):
            ...
            if math.isinf(r["tau"]):
                ...
                annot[i, j] = f"{r['FPR_post_warmup']:.2f}\n{int(r['detection_delay_s'])}s"
    ...
    ax.set_title(f"warmup = {warmup}s ({warmup//60} min pre-attack)",
                 fontsize=10.5, pad=8)
...
cbar.set_label("Detection delay (s) \u2014 color capped at 1200s; "
               "actual delay annotated in cell",
               fontsize=9, labelpad=10)

fig.suptitle("Tau-recalibration sweep on real CIC-IDS2017 Friday DDoS\n"
...
for w in WARMUPS:
    cells = []
    for k in KS:
        det = sum(1 for r in rows if r["warmup_s"] == w and r["k"] == k and r["detected"])
...
best = None
for r in rows:
    if r["detected"] and r["detection_delay_s"] <= 300 and (
            best is None or r["FPR_post_warmup"] < best["FPR_post_warmup"]):
        best = r
if best:
    print(f"  {best['estimator']}(W={best['W']},s={best['s']})  "
